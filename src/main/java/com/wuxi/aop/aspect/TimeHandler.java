...
public class TimeHandler {
	...
	@After("execution(* com.wuxi.aop.aspect.HelloWorld.b*(..)) and args(name)")
	public void printB(JoinPoint joinpoint,String name){
		System.out.println("参数： " + name);
		System.out.println("连接点类型： " + joinpoint.getKind());
		System.out.println("连接点的目标类： "+joinpoint.getTarget().getClass().getName());
		System.out.println("代理类： " + joinpoint.getThis().getClass().getName());
		System.out.println("连接点类型： " + joinpoint.getClass().getName());
		System.out.println("连接点：" + joinpoint.getStaticPart().toLongString());
		System.out.println("连接点的目标方法签名： " + joinpoint.getSignature().getName());
		
		MethodSignature signature =  (MethodSignature) joinpoint.getSignature();
		Method method = signature.getMethod();
		System.out.println("连接点的方法：" + method.getName());
		System.out.println("拦截 bye after");
	}
}
